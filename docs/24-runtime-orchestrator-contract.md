...
- [x] **Emergency-transfer SIP handoff, wired but unverified against a live call** — `escalateEmergency`'s `action: "forward_call"` now carries `transferTargets` (resolved via `ResolveOnCallUseCase`) all the way to `apps/voice-runtime`, which executes it via LiveKit's `WarmTransferTask`. Requires a separately-provisioned LiveKit *outbound* SIP trunk ([27-voice-runtime-provisioning.md](27-voice-runtime-provisioning.md) §5) — verify against a real SIP trunk and a real on-call phone number before relying on this in a live emergency call.
