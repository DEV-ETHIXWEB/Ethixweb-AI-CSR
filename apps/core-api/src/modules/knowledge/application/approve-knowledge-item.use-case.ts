...
export interface ApproveKnowledgeItemCommand {
  tenantId: string;
  itemId: string;
...
@Injectable()
export class ApproveKnowledgeItemUseCase {
  constructor(
  ...
  async execute(command: ApproveKnowledgeItemCommand): Promise<KnowledgeItem> {
    setSpanAttributes({
    ...
    return this.tenantContext.run(command.tenantId, async (db) => {
      ...
      assertValidKnowledgeStatusTransition(command.itemId, existing.status, "approved");
