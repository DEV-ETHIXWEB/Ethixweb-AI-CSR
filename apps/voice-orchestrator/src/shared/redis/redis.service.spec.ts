 ...
 * indication Redis was the actual problem, notable here in particular
 * because this service is entirely Redis-backed (conversation state,
 * capacity, idempotency, tool audit log all depend on it, unlike core-api
 * where Redis is one dependency among several). Fixed with an explicit,
 ...
 * before re-throwing. These tests never make a real network call:
 * connect() is stubbed on the constructed instance before onModuleInit
 * runs.
 */
