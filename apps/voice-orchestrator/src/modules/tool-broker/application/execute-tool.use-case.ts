...
export class ExecuteToolUseCase {
  ...
  async execute(command: ExecuteToolCommand): Promise<ToolExecutionResult> {
    ...
    // No tool schema declares business_id/call_id (see tool-catalog.ts) —
    // those are facts about the call, not model decisions, and are passed
    // to the handler below via ToolHandlerContext instead, sourced only
    // from this trusted command, never from model output.
