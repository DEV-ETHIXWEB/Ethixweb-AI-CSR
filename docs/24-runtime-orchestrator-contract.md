...
- [ ] **Set `ORCHESTRATOR_SERVICE_TOKEN`** to a real generated secret (not the `.env.example` placeholder) in both `apps/voice-runtime`'s config and voice-orchestrator's deployed environment — they must match exactly, it's a shared secret, not an issued credential.
- [x] **`conversationId` discovery after restart** — `GET /v1/conversations/by-call/:callId` now exists (closes §5's gap below). `apps/voice-runtime`'s own reconnect logic doesn't call it yet on room-disconnect recovery — that piece is still open, tracked in [13-implementation-backlog.md](13-implementation-backlog.md) item 8.
- [x] **Idempotency key generation, confirmed correct** — `apps/voice-runtime/src/call-session.ts`'s `runTurn` generates the key exactly once per turn attempt; `orchestrator-client.ts`'s retry wrapper reuses that identical value across any transient retry (network error, 5xx, 409). Unit-tested in `orchestrator-client.spec.ts` and `call-session.spec.ts`.
...
- [ ] **Load-test the barge-in path** (§2.3) against real STT/VAD timing — `apps/voice-runtime` wires both mechanisms (mid-turn `AbortController`, between-turns `POST /:id/interrupt` via `AgentSession`'s `UserStateChanged` event), unit-tested against mocks, but actual latency characteristics under a live telephony connection remain unverified — no live LiveKit/Twilio session available in this environment.
- [x] **Emergency-transfer SIP handoff, wired but unverified against a live call** — `escalateEmergency`'s `action: "forward_call"` now carries `transferTargets` (resolved via `ResolveOnCallUseCase`) all the way to `apps/voice-runtime`, which executes it via LiveKit's `WarmTransferTask`. Requires a separately-provisioned LiveKit _outbound_ SIP trunk ([27-voice-runtime-provisioning.md](27-voice-runtime-provisioning.md) §5) — verify against a real SIP trunk and a real on-call phone number before relying on this in a live emergency call.

## 5. Known gap, deliberately out of scope for this phase

~~No HTTP route exposes `ConversationRepository.findByCallId`~~ — closed: `GET /v1/conversations/by-call/:callId?tenantId=` now exists (`ConversationsController.findByCallId`, backed by `GetConversationByCallIdUseCase`), verified in `runtime-contract.e2e.spec.ts`'s "lookup by callId" suite.
